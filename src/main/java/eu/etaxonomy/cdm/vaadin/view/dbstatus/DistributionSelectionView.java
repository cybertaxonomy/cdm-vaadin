...
public class DistributionSelectionView extends CustomComponent implements IDistributionSelectionComponent, View, ClickListener{
	...
	public void buttonClick(ClickEvent event) {
		if(classificationTree!=null && distributionAreaBox != null){
			TaxonNode taxonNode = (TaxonNode) classificationTree.getValue();
			TermVocabulary<DefinedTermBase> term = (TermVocabulary<DefinedTermBase>)distributionAreaBox.getValue();
			try {
				distListener.buttonClick(null, term);
	...
	public void dataBinding(){
		...
		for(Classification classification:classificationList){
			...
			classificationTree.addItem(rootNode);
			classificationTree.setItemCaption(rootNode, rootNode.getClassification().getName().toString());
			classificationTree.setParent(rootNode, null);
	...
	private void addChildNodes(TaxonNode parentNode) {
		...
		if(childNodes.size()==0){
			...
			classificationTree.setChildrenAllowed(parentNode, false);
		}
		for (TaxonNode taxonNode : childNodes) {
			if(taxonNode!=null){//TODO when does this happen?? orphaned taxa??
				classificationTree.addItem(taxonNode);
				classificationTree.setItemCaption(taxonNode, taxonNode.getTaxon().getName().getTitleCache());
				classificationTree.setParent(taxonNode, parentNode);
    ...
    private VerticalLayout buildVerticalLayout_2() {
        ...
        //classification tree
        classificationTree = new Tree();
        verticalLayout_2.addComponent(classificationTree);
